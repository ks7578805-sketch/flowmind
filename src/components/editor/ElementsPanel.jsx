import { Type, Image, Star, Volume2, Video, Minus, GitFork, Share2, LayoutList, Circle, Grid3X3, Plus, Sparkles, AlignLeft, Heading1, Lightbulb } from 'lucide-react';
...
export default function ElementsPanel({ onAddNode, onOpenGlows }) {
  ...
  return (
    <div className="w-44 bg-[#0d0d0d] border-r border-white/5 flex flex-col overflow-y-auto">
      ...
      <div className="p-3 border-t border-white/5">
        <button
          onClick={onOpenGlows}
          className="w-full flex items-center gap-2 px-2 py-2 rounded-lg bg-white/3 border border-white/5 hover:bg-white/8 hover:border-primary/30 transition-all group"
        >
          <Lightbulb className="w-3.5 h-3.5 text-yellow-400/70 group-hover:text-yellow-300 transition-colors" />
          <span className="text-[10px] text-muted-foreground group-hover:text-white transition-colors">Brilhos de Fundo</span>
        </button>
      </div>
    </div>
  );
